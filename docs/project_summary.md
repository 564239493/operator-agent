...
> 生成日期: 2026-05-28 | 基于代码实际状态，非 CLAUDE.md 中的理想架构
...
**当前完成度**：约 35%，文档解析、数据库层、基础 Pipeline 可运行。约束提取、人工审核、测试用例生成尚未实现。
...
│   │       ├── server.py            # FastMCP 实例，注册 14 个 tool + 2 个 resource
│   │       ├── parsers/
...
│           │   ├── param_desc_extract.py # 节点3：双Section路由 + LLM并发提取参数描述
│           │   └── shape_extract.py     # 节点4：按函数分组 + 批量LLM提取无条件shape
│           ├── routes/
...
│           ├── prompts/
│           │   ├── __init__.py      # PRODUCT_SUPPORT_EXTRACT_PROMPT, PARAM_DESC_EXTRACT_PROMPT, SHAPE_EXTRACT_PROMPT
...
parse_params_node(state)
  ...
  │    └─ 每个函数-参数对 → 一条记录
  │    └─ direction 默认为 ""（稍后由 param_desc_extract 从 Markdown 表格提取）
  │
...
param_desc_extract_node(state)
  │
  ├─ MCP: query_params_by_doc_id(doc_id) → 当前版本的参数列表
  │    └─ 如无参数，直接返回
  │
  ├─ MCP: get_section(doc_id, "params_get_workspace") → GetWorkspaceSize 参数说明
  │    MCP: get_section(doc_id, "params_execute")      → Execute 函数参数说明
  │
  ├─ 按函数名拆分参数：
  │    ├─ ws_params: function_name 以 "GetWorkspaceSize" 结尾 → 使用 params_get_workspace 内容
  │    └─ exe_params: 其他函数                                        → 使用 params_execute 内容
  │    └─ 如对应 section 不存在，跳过该组参数并 logger.warning（不使用 fallback）
  │
  ├─ 并发 LLM 调用（asyncio.Semaphore(5)）:
  │    └─ 对每个参数并行调用（最多 5 个并发）:
  │       LLM: ChatOpenAI(PARAM_DESC_EXTRACT_PROMPT.format(param_name, section_content))
  │       └─ 温度 0.1，生成标准 Markdown 表格描述
  │       └─ _parse_direction(): 用正则从表格 "| 输入/输出 | 输入/输出 |" 行提取 direction
  │
  └─ MCP: update_param_descriptions(doc_id, updates)
       └─ 批量 UPDATE parameters 表的 direction、description 等 7 个字段
```

**输出状态**：`error`

**注意**：`_extract_one()` 中 `usage_notes`, `data_type`, `data_format`, `shape`, `memory_desc` 仍硬编码为空字符串。其中 `shape` 由下游 `ShapeExtract` 节点填充，其余字段暂无提取逻辑。

---

#### 节点4: shape_extract_node（顺序执行，等待节点3完成）

**触发条件**：param_desc_extract_node 完成后执行。

**内部流程**：

```
shape_extract_node(state)
  │
  ├─ MCP: query_params_by_doc_id(doc_id) → 所有参数（含刚生成的 Markdown 描述）
  │    └─ 筛选 description 非空的参数
  │
  ├─ 按 function_name 分组 → 每组一次批量 LLM 调用
  │    └─ 将同组所有参数的描述打包为 params_text
  │
  ├─ 并发 LLM（asyncio.Semaphore(5)，每组上限 15 参数）:
  │    └─ LLM: ChatOpenAI(SHAPE_EXTRACT_PROMPT.format(params_text=...))
  │       └─ 温度 0.1，分析 "维度(shape)"、"使用说明"、"描述" 行
  │       └─ 判断是否有前置条件（依赖其他参数取值）
  │       └─ 无条件 → 提取 shape 值（如 "1-8"、"(N, C, H, W)"）
  │       └─ 有条件（含"当.../如果.../若..."）→ shape 为空，忽略
  │       └─ 返回 JSON: [{"param_name": ..., "shape": ...}, ...]
  │
  └─ MCP: update_param_shape(doc_id, shape_updates)
       └─ 专用工具，SQL 只更新 shape 列，不触碰 direction/description 等字段
```

**输出状态**：`error`

**与节点3的区别**：节点3逐参数调用 LLM 生成描述，节点4按函数批量调用 LLM 提取 shape。节点4使用专用的 `update_param_shape` MCP 工具，避免覆盖节点3写入的 direction 字段。
...
t4:                                 ▼
                           param_desc_extract
                           (双Section路由 + LLM并发提取参数描述)
                                     │
t5:                                 ▼
                           shape_extract
                           (按函数分组 + 批量LLM提取无条件shape)
                                     │
t6:                                 ▼
                                   END
```

**MCP 通信模式**：每个节点每次调用 MCP 工具时，`MCPClient._call_tool()` 会**启动一个新的 MCP Server 子进程**（`async with stdio_client`），调用完成后进程退出。整个 Pipeline 执行一次约产生 12-15 次子进程启动（含 ShapeExtract 节点的 MCP 调用）。
...
| POST | `/api/v1/upload` | 上传算子文档，触发完整 Pipeline | init_doc → [product_support ∥ parse_params] → param_desc_extract → shape_extract |
...
4. **`param_desc_extract_node` 部分覆盖**：`_extract_one()` 中 `direction` 已通过正则从 Markdown 表格提取（不再覆盖），`shape` 由下游 `ShapeExtract` 节点填充。但 `usage_notes`、`data_type`、`data_format`、`memory_desc` 仍硬编码为空字符串
...
### 前端
11. ~~**字段名不匹配**~~ ✅ 已修复 — MCP 层 dict key 从 `dtype_desc`/`dformat_desc` 重命名为 `data_type`/`data_format`
12. **Markdown 渲染与双过滤器**：已集成 marked.js CDN，"描述"列渲染 Markdown 格式。新增函数名下拉过滤器，与算子过滤器联动，客户端过滤。
